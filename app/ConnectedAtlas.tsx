...
import { itemEvidence, itemStatusLabel, publishableItems, publishableStats, recipes, sourceFor, sourceStatusLabel, statusLabel } from "../lib/catalog";
...
function ItemAtlasDetail({ node, selectNode, nodeFor }: { node: AtlasNode; selectNode: (node: AtlasNode) => void; nodeFor: (type: AtlasNode["type"], key: string) => AtlasNode | undefined }) {
  ...
  const visualSource = visual ? sourceFor(visual.sourceId) : null;
  ...
  return <div className="atlas-item-detail">
    {visual && <div className={`atlas-item-image ${visual.kind === "item_icon" ? "item-icon-reference" : ""} ${visual.exactItem ? "" : "set-reference"}`}><Image src={visual.url} alt={visual.alt} width={visual.width} height={visual.height} unoptimized={visual.unoptimized} style={visual.focus ? {objectPosition:visual.focus,width:"100%",height:"100%",objectFit:"cover"} : undefined}/><span>{visual.label}</span></div>}
    ...
    <div className="atlas-source-row">{(itemSource || recipeSource || visualSource) && <Link href="/kaynaklar#esyalar">Eşya ve reçete kaynaklarını kategori içinde gör →</Link>}</div>
  </div>;
}
