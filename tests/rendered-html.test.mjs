...
test("Nefer Atlası kabuğunu ve varsayılan donanım modülünü oluşturur", async () => {
  ...
  for (const tab of ["Build", "Tılsım", "Bölgeler", "Görevler", "Eşyalar", "Atlas", "Endgame", "Maden", "Döngü", "Yetenek", "Sorunlar", "Gelişim", "Katkı"]) {
  ...
  assert.match(html, /BETA(?:<!-- -->)? v(?:<!-- -->)?0\.31\.0/);
...
test("bağlantıyla erişilen rehber, kullanım akışlarını ve güven sözlüğünü gösterir", async () => {
  ...
  for (const moduleName of ["Build", "Tılsım", "Bölgeler", "Görevler", "Eşyalar", "Atlas", "Endgame", "Maden", "Döngü", "Yetenek", "Sorunlar", "Gelişim", "Katkı"]) {
