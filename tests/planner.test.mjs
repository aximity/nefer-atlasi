...
test("eşya toplamı çelişkili alanı dışarıda bırakıp uyumlu değerleri toplar",()=>{const result=sumPublishedStats(["sifir-kelvin-ceket","sifir-kelvin-eldiven"],stats);assert.equal(result.values["Büyü Hasarı (Buz)"],150000);assert.equal(result.values["Maksimum Kudret"],440000);assert.equal(result.values["Büyü Kritik Şansı"],8708);assert.deepEqual(result.incompatible,[])});
