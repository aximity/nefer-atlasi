...
test("genel durum reçete claimi ile etkilenen reçete sayısını karıştırmaz", () => {
  ...
  assert.equal(projectLiveFacts.progressionObservationCount, 18);
...
test("proje durumu sürüm ve çapraz denetim standardını yayımlar", () => {
  assert.equal(SITE_RELEASE.version, "0.77.0");
  assert.equal(SITE_RELEASE.milestone, "M77");
  assert.equal(SITE_RELEASE.releasedOn, "2026-09-05");
