...
export type FarmSessionInput = {server:string;region:string;routeName:string;profession:string;observedAt:string;durationMinutes:number;nodeCount:number;boosterProfile:string;gameCost:number;tlCostKurus:number;notes:string;routeTemplateId:string|null;yields:FarmYield[]};
