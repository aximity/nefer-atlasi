...
export function productionDraftImpact({ recipes = [], items = [], stock = {}, draft = {}, targets = {}, favoriteIds = [] }) {
  ...
  const newlyReadyIds = new Set(newlyReady.map((plan) => plan.recipe.itemId));
  const ranked = [...after]
    .filter((plan) => plan.completion > 0)
    .sort((a, b) =>
      Number(newlyReadyIds.has(b.recipe.itemId)) - Number(newlyReadyIds.has(a.recipe.itemId))
      || Number(b.status === "ready") - Number(a.status === "ready")
      || Number(favorites.has(b.recipe.itemId)) - Number(favorites.has(a.recipe.itemId))
      || b.completion - a.completion
      || a.missing.length - b.missing.length
      || a.recipe.itemId.localeCompare(b.recipe.itemId, "tr"),
    );
  const near = ranked.filter((plan) => plan.status === "ready" || plan.completion >= 50);
  const recommendations = (near.length >= 3
    ? near
    : [...near, ...ranked.filter((plan) => !near.includes(plan))]
  ).slice(0, 5);
  return {
    mergedStock,
