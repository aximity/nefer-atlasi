...
const crossVerifiedCemberlitasIds=new Set(cemberlitasEvidenceRows.flatMap(group=>group.itemIds));
const cemberlitasItems=itemRows.map(item=>crossVerifiedCemberlitasIds.has(item.id)?{...item,publicationStatus:"cross_verified",lastChecked:"2026-08-28"}:item);
...
export const evidence = [...evidenceRows,...cemberlitasEvidence,...groupLootEvidence,...glassesEvidence] as EvidenceClaim[];
