...
const CONTRIBUTION_EVIDENCE_GUIDANCE = {
  conflict: "Uyuşmayan alanı ve doğru değeri gösteren kaynak bağlantısını veya oyun içi ekran görüntüsünü ekle.",
  acquisition: "Elde etme yerini, yaratığı, NPC'yi ya da reçeteyi açıkça gösteren kaynak bağlantısını veya oyun içi ekran görüntüsünü ekle.",
  stats: "Eşya adı ile özellik satırlarının aynı oyun içi görüntüde okunması gerekir.",
  material_source: "Malzeme adı ile edinme yöntemini birlikte gösteren kaynak bağlantısını veya oyun içi ekran görüntüsünü ekle.",
  media: "Ad ile görünüşün aynı oyun içi ekran görüntüsünde birlikte okunması gerekir; yalnız kırpılmış ikon yeterli değildir.",
  verification: "Mevcut kayıttan bağımsız ikinci kaynağın bağlantısını veya oyun içi ekran görüntüsünü ekle.",
};

export function completionContributionHref(record) {
  const params = new URLSearchParams({
    module: "contribute",
    subject: `Kanıt görevi · ${record.name}`.slice(0, 120),
    comment: [
      `${COMPLETION_KIND_LABELS[record.kind] ?? "Eksik kayıt"}: ${record.subtitle}`,
      record.detail,
      CONTRIBUTION_EVIDENCE_GUIDANCE[record.kind] ?? "Kaynağı veya oyun içi kanıtı açıklamaya ekle.",
    ].filter(Boolean).join("\n\n").slice(0, 2000),
  });
  return `/?${params.toString()}#contribute`;
}
...
export function buildAtlasCompletionQueue({
  ...
  for (const family of additionalVisualFamilies) {
    ...
    records.push({
      ...
      href: isTalisman ? "/?module=engine#modules" : "/?module=recipes&kind=potion#recipes",
    });
  }
