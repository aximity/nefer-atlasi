...
export function buildAtlasCompletionQueue({
  ...
  for (const family of additionalVisualFamilies) {
    ...
    records.push({
      ...
      entityId: family.id,
      name: family.label,
      subtitle: isTalisman ? "179 tılsım · iki ortak renk ailesinden biri" : "246 iksir · üç ortak renk ailesinden biri",
  ...
  for (const node of graph.materialNodes) {
    if (!node.source) {
      records.push({
        ...
        detail: "Doğrulanmış toplayıcılık, üretim veya yaratık ganimeti kaynağı eşleşmedi; bölge ya da meslek tahmin edilmedi.",
      });
    } else if (node.source.kind === "talisman_acquisition") {
      records.push({
