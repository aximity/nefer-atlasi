...
export function buildAtlasGraph({ items = [], recipes = [], materialSourceFor }) {
  ...
  const itemNodes = items.map((item) => {
    ...
    const derivedCemberlitas = recipe?.sourceId === "maxigame-cemberlitas-2015";
    return {
      id: `item:${item.id}`,
      key: item.id,
      type: "item",
      ...
      boss: item.boss || (derivedCemberlitas ? "Gaffar Bey" : null),
      verificationStatus: item.publicationStatus,
      searchText: normalize(`${item.name} ${item.class} ${item.slot} ${item.region || ""} ${item.boss || ""}`),
    };
  });
  ...
  for (const recipe of recipes) {
    ...
    for (const material of recipe.materials || []) {
      ...
      current.uses.push({ itemId: item.id, itemName: item.name, quantity: material.quantity, recipeId: recipe.id, verificationStatus: recipe.verificationStatus });
  ...
  for (const node of itemNodes) {
    ...
    const key = normalize(`${node.region || ""}|${node.boss}`);
    const current = bossMap.get(key) || { boss: node.boss, region: node.region, itemIds: [] };
  ...
  for (const node of itemNodes) {
    ...
    if (node.boss) current.bosses.add(node.boss);
