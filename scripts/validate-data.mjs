...
const items=read("items.json"),stats=read("stats.json"),recipes=read("recipes.json"),sources=read("sources.json"),evidence=read("evidence.json"),images=read("images.json"),talismans=read("talismans.json"),contexts=read("contexts.json");
...
[["items",items],["stats",stats],["recipes",recipes],["sources",sources],["evidence",evidence],["talismans",talismans],["contexts",contexts]].forEach(([n,r])=>unique(r,n));
const itemIds=new Set(items.map(x=>x.id)),sourceIds=new Set(sources.map(x=>x.id));
const allowed={class:["Savaşçı","Büyücü","Şifacı"],slot:["Gözlük","Ceket","Eldiven","Pantolon","Ayakkabı","Zırh","Amplifikatör","Yüzük","Kolye","Silah"],rarity:["Şaheser"],status:["draft","single_source","cross_verified","conflicted"],source:["official","forum","fandom","video","player_screenshot"]};
...
for(const i of items)for(const field of ["name","class","level","slot","rarity","appearanceFamily"])if(!evidence.some(e=>e.itemId===i.id&&e.field===field&&e.checkedAt&&sourceIds.has(e.sourceId)))errors.push(`${i.id}: ${field} için tarihli/kaynaklı kanıt yok`);
...
for(const t of talismans)if(!sourceIds.has(t.sourceId)||t.class!=="Şifacı"||!t.color||![1,2,3].includes(t.tier)||!t.requiresBase||!allowed.status.includes(t.status))errors.push(`${t.id}: eksik/geçersiz tılsım modeli`);
...
if(items.length!==67)errors.push(`tam Çemberlitaş denetimi 67 yerine ${items.length} kayıt içeriyor`);
